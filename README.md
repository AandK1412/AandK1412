# hey, I'm Anrunya 👋 &nbsp;[![LinkedIn](https://img.shields.io/badge/-connect-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/anrunya-patole-817126269/) [![Email](https://img.shields.io/badge/-email-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:anrunya@gmail.com)

> 📊 M.Sc. Business Analytics @ Virginia Tech · turning raw data into "aha!" moments · Winchester, VA

---

### 🛠️ what I work with

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![Power BI](https://img.shields.io/badge/Power%20BI-F2C811?style=flat-square&logo=powerbi&logoColor=black)
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=flat-square&logo=tableau&logoColor=white)
![SAS Viya](https://img.shields.io/badge/SAS%20Viya-002F5F?style=flat-square&logo=sas&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white)
![Power Automate](https://img.shields.io/badge/Power%20Automate-0066FF?style=flat-square&logo=microsoftpowerautomate&logoColor=white)
![VBA](https://img.shields.io/badge/VBA-217346?style=flat-square&logo=microsoftexcel&logoColor=white)

---

### 🚀 things I've built

| project | what it does | stack |
|---|---|---|
| 🏆 **Climate Dashboard** *(1st place)* | 200k+ records → 30-yr trend story | Python · Tableau |
| 📣 **Sales Campaign Analysis** | ETL pipeline + conversion optimization | Python · SAS Viya |
| 🏗️ **OCR Extraction System** | digitized architectural drawings, 70% faster | Python · OCR |
| 🎓 **Facial Recognition Attendance** | real-time face detection → auto attendance log | Python · ML |

---

### 📈 github stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=YOUR_GITHUB_USERNAME&show_icons=true&hide_border=true&count_private=true&hide_title=true" height="140"/>
  &nbsp;
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_GITHUB_USERNAME&layout=compact&hide_border=true" height="140"/>
</p>

---

### 🌱 right now

- 🔍 hunting for entry-level **Data Analyst** roles in the US *(F-1 OPT)*
- 📚 levelling up on cloud data tools & advanced SQL
- 🤝 volunteering with **Spark-A-Change** — teaching kids to build with tech

---

<p align="center"><sub>👀 poke around my repos · ⭐ star anything useful · 📬 always happy to chat data</sub></p>
